...
(function (w) {
	...
	json_object.ext({
		...
		getType: function (op) {
			if (op != undefined) {
				var cName = op.constructor.name
				if (cName === undefined) { // IE does not support constructor.name
					cName = op.constructor.toString().match(fname)[1];
				}
				return cName;
			} else {
				return null;
	...
	function json_view(json_object) { // depends upon json_object object 
		var _this = this,
			html = "",
	...
	json_view.ext({
		...
		option: function (json_object) {
			var _Object = "<button class='op _Object'>{} Object</button>",
				_Array = "<button class='op _Array'>[] Array</button>",
				_String = "<button class='op _String'>\"\" String</button>",
				_Number = "<button class='op _Number'>0 Number</button>",
				_Boolean = "<button class='op _Boolean'>Boolean</button>",
				html = "";
	...
	window.json_object = json_object;
})(window);